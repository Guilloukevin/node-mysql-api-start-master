...
if (findEmail[0].cnt > 0)return res.status(400).json({message: "l'email existe deja"})
...
exports.Login = async (req, res) => {
  ...
  const user = await querysql('SELECT id, email, password FROM user WHERE email = ?', email)
  const passwordCheck = await bcrypt.compare(password, user[0].password)
  if(!passwordCheck) return res.status(400).json({message:'Mot de passe incorrect'})
